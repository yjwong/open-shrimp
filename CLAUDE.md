...
src/open_shrimp/
    ...
    events/                    # Inbound events: sources -> per-source forum topics
        ...
        schedule.py            # Scheduled tasks: JobQueue registration + per-task-topic run flow
        manager.py             # Start/stop adapters and the schedule runner with the bot lifecycle
...
- `contexts` - Map of context name -> {directory, description, model, effort, allowed_tools, disallowed_tools, default_for_chats, locked_for_chats, additional_directories, sandbox}
