...
- **Sudo mode** (`sandbox.allow_host_escape: true`): Opt-in per-context escape hatch that registers a `host_bash` MCP tool, which runs shell commands on the HOST (outside the sandbox), `cwd` set to the context's source directory. Every invocation needs a fresh, intentional approval — no pattern rules or session blanket approvals; an unanswered prompt auto-denies with a "timed out" message (distinct from explicit denial) so the agent can retry or fall back to sandboxed `Bash`. The `host_bash` check in `hooks.py` runs before all other approval checks, so the sandbox auto-approve path for Bash never bypasses it. All outcomes are audited to `sudo.log` (mode 0600) under the data directory.
