...
- **Sandbox**: Optional per-context isolated execution via the `sandbox:` config key (the older `container:` key is a backwards-compatible alias for `sandbox.backend: docker`). Multiple backends are supported:
  ...
  - **Persistent volumes** (libvirt only): The `persistent_paths` config key lists guest paths (e.g. `/var/lib/docker`) that get dedicated qcow2 disk images. These volumes survive VM rebuilds (only the overlay is deleted). Each volume is 100 GB sparse (thin-provisioned — actual host disk usage matches written data). Auto-formatted ext4 on first use, mounted via systemd units with `LABEL=` for device-order independence. `discard=unmap` on both the virtio disk and ext4 mount enables automatic space reclamation when files are deleted.
- **Sudo mode** (`sandbox.allow_host_escape: true`): Opt-in per-context escape hatch that registers a `host_bash` MCP tool. The tool runs shell commands on the HOST (outside the sandbox) with `cwd` set to the context's source directory — same tree as inside the sandbox, just unsandboxed. Every invocation routes through a dedicated approval flow in `handlers/approval.py:_send_host_bash_approval` with `[Approve][Deny]` buttons and a live countdown; if the user doesn't respond within 10 seconds the call is auto-denied and the agent receives a "timed out" deny message (distinct from explicit user denial) so it can decide whether to retry later or fall back to the sandboxed `Bash` tool. No pattern rules or session blanket approvals are available — every host command needs a fresh, intentional approval. The host_bash special case in `hooks.py` runs before all other approval checks, so the containerized auto-approve path for Bash never bypasses it. All outcomes are appended to `sudo.log` under the OpenShrimp data directory resolved by `paths.data_dir()` (platform-specific via `platformdirs`, instance-scoped when `instance_name` is set), with mode 0600, by `sudo_audit.py` for audit.
- **Computer use**: Optional per-context GUI interaction via `sandbox.computer_use: true`. Runs a headless Wayland desktop (labwc compositor, 1280x720) inside the sandbox with Chromium and a foot terminal. Claude interacts via MCP tools: `computer_screenshot` (grim), `computer_click`/`computer_type`/`computer_key`/`computer_scroll` (wlrctl), and `computer_toplevel` for window management. Screenshots are saved to a bind-mounted directory and sent to Telegram for user observability. A VNC server (wayvnc) is exposed on a dynamic port for live viewing; the `/vnc` command opens a noVNC Mini App. The computer-use image extends the matching base image with a parametric `Dockerfile.computer-use` — two tags (`openshrimp-computer-use:latest` for Claude, `openshrimp-opencode-computer-use:latest` for OpenCode), selected from the bundle's `computer_use_image` and `computer_use_build_args`.
