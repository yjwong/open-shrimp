...
from open_udang.container import ensure_image, CONTAINER_IMAGE


def test_ensure_image_skips_when_image_exists():
    """When the image already exists, ensure_image does nothing."""
    with patch("open_udang.container.subprocess.run") as mock_run:
...
def test_ensure_image_builds_when_missing(tmp_path):
    ...
    with (
        patch("open_udang.container.subprocess.run", mock_run),
        patch("open_udang.container.subprocess.Popen", mock_popen),
        patch("open_udang.container.find_claude_binary", return_value=str(fake_binary)),
    ):
        ensure_image()
