...
Run OpenCode inside an isolated environment. When a sandbox is enabled, all Bash commands and path-scoped tools are auto-approved since the sandbox provides the safety boundary.
...
sandbox:
  ...
  dockerfile: /path/to/Dockerfile.opencode
  computer_use: true
```
...
- Custom Dockerfiles should `FROM openshrimp-opencode:latest`; images are tagged `openshrimp-opencode:<context-name>` and built lazily
