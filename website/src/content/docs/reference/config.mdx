...
:::note
The context-level `backend` (agent runtime) is unrelated to `sandbox.backend` (sandbox type). The `opencode` backend has extra preconditions: provider-qualified models (`provider/model`), out-of-band `opencode auth login`, and a discoverable `opencode` binary. See [Agent Backends](/guides/backends/).
:::
