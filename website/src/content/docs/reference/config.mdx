...
| `default_context` | string | No | `null` | Context a chat is bound to when it has not picked one |
| `backend` | string | No | `claude_sdk` | Agent runtime: `claude_sdk` or `opencode`. Distinct from `sandbox.backend` (the sandbox type). See [Agent Backends](/guides/backends/) |
...
:::tip
The setup wizard fills this in for you, by having you message the bot and type
back the code it replies with. To add a second account by hand, send `/start` to
[@userinfobot](https://t.me/userinfobot) to find its numeric user ID.
:::
...
| `backend` | string | No | inherits top-level | Per-context override of the agent runtime (`claude_sdk` or `opencode`). Inherits the top-level `backend` when omitted. This is the agent runtime — distinct from `sandbox.backend` (the sandbox type) |
| `sandbox` | object | No | `null` | Sandbox configuration (see below) |

:::note
The context-level `backend` (agent runtime) is unrelated to `sandbox.backend` (sandbox type). The `opencode` backend has extra preconditions: provider-qualified models (`provider/model`) and an out-of-band `opencode auth login`. The CLI is downloaded on first use. See [Agent Backends](/guides/backends/).
:::
...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `backend` | string | Yes | — | `"libvirt"` (Linux), `"lima"` (macOS), or `"hcs"` (Windows) |
...
| `memory` | int | No | `2048` | Memory ceiling in MB (Libvirt, Lima, and HCS backends) |
| `cpus` | int | No | `2` | Number of vCPUs (Libvirt, Lima, and HCS backends). HCS rejects a count above the host's logical processor count |
| `disk_size` | int | No | `20` | Disk size in GB for the qcow2 overlay (Libvirt, Lima) or each dynamic VHDX volume (HCS) |
| `base_image` | string | No | `null` | Path to a base image you supply — qcow2 for Libvirt, VHDX for HCS. Both backends download one when unset |
| `provision` | string | No | `null` | Shell script to run on first boot (Libvirt, Lima, and HCS backends) |
...
The context a chat is bound to when it has not picked one. Optional — when
omitted, a chat is asked to choose a project instead of being bound to one.
When set it must match a key in `contexts`.

`contexts` may itself be empty, which is what a fresh install looks like
before its first project is added.
