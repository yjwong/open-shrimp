...
contexts:
  myproject:
    ...
    sandbox:
      ...
      memory: 4096             # guest memory ceiling in MB
      provision: |             # shell script run on the guest's first boot
...
See the dedicated guides for [VM sandbox](/guides/vm-sandbox/), [Lima sandbox](/guides/lima-sandbox/), and [HCS sandbox](/guides/hcs-sandbox/).

## Agent backend

The top-level `backend:` key selects the agent runtime. The default is `claude_sdk` (the Claude Agent SDK). Set it to `opencode` to use [`sst/opencode`](https://github.com/sst/opencode) instead, which supports OpenAI, Anthropic, and Google models:
...
OpenCode requires provider-qualified models and an out-of-band `opencode auth login`. The CLI itself is downloaded on first use. See [Agent Backends](/guides/backends/) for full setup.
