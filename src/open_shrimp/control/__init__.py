...
:func:`open_shrimp.main.request_shutdown` in-process the way the macOS
menu-bar app does.  Without it the only way to stop the core from another
process on Windows is ``TerminateProcess``, which skips shutdown entirely and
strands any running sandbox guest.
"""
