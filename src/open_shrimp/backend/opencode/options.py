...
Two seams: neither ``endpoint`` nor ``handle_questions`` is a
``BackendOptions`` field, so both ride in ``opts.extra``:
...
def to_opencode(opts: BackendOptions) -> OpenCodeOptions:
    ...
    ``handle_questions`` are read from ``extra`` (they are not honoured-
    intersection fields).  SDK-only fields are carried through but never read.
    """
